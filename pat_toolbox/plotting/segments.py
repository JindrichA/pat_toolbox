...
def _add_segment_pages_to_pdf(
    ...
    *,
    ...
    for start in range(0, n_samples, samples_per_segment):
        ...
        n_rows = 2 + (1 if use_hrv else 0) + (1 if use_pat_amp else 0)
        ...
        ax_pat = axes[0]
        ax_hr = axes[1]
        ax_hrv = axes[2] if use_hrv else None
        ax_amp = axes[3] if (use_hrv and use_pat_amp) else (axes[2] if (use_pat_amp and not use_hrv) else None)
